	...
	showHistoryVersion: function(a) {
		...
		$.ajax({
			...
			success: function(b) {
				...
				$("#btn_history_restore").button("enable");
				UI.showTip("您在正浏览一个历史版本<a href='javascript:' style='margin-left: 10px;color:#0080FF' onclick='Dock.closeHistory()'>点击退出</a>")
			}
		})
	},
...
var ProShare = {
	publish: {
		...
		execute: function(e) {
			...
			function b(l) {
				...
				if (l != "private") {
					if (m.description.length < 15) {
						Util.globalTopTip("请输入文件描述，至少15个字", "top_error", 3000, $("#pubpo_win"), true);
						g.focus();
						return
					}
					if (m.tags.length == 0) {
					...
					if (j) {
						...
						if (i.length == 0) {
							...
							if (!/^(\d+\.\d{1,1}|\d+)$/.test(i)) {
								...
								if (i < 0.1) {
									Util.globalTopTip("您输入的克隆价格小于规定范围", "top_error", 3000, $("#pubpo_win"), true);
									k.focus();
									return
								}
							}
	...
	viewlink: {
		...
		execute: function(i) {
			...
			$(".share_to_a").off("mousedown").on("mousedown", function(k) {
				k.stopPropagation();
				var j = $(this);
				Util.ajax({
					url: "/view/getlink",
					...
					success: function(o) {
						if (o.viewLinkId != "") {
							var m = j.attr("hrefs");
							if (m.indexOf("/view/link/") < 0) {
								j.attr("hrefs", m.replace("=https://www.processon.com", "=" + o.viewLinkId))
							}
							if (j.hasClass("weixin")) {
								var p = j.attr("hrefs").replace("https://www.processon", "http://test.processon");
			...
			function b(j) {
				...
				var k = l.attr("val");
				if (k == "false") {
					l.removeClass("green").addClass("gray");
					l.find(".switch").removeClass("right").addClass("left");
					l.find(".switch .icons").show();
					l.find(".switch-left").hide();
					l.find(".switch-right").show()
				} else {
					l.removeClass("gray").addClass("green");
					l.find(".switch").removeClass("left").addClass("right");
					l.find(".switch .icons").hide();
					l.find(".switch-left").show();
					l.find(".switch-right").hide();
					if ( !! j) {
						$(".input-pw").show().val(j)
					}
				}
			...
			function c(l) {
				var k = $(l).attr("val");
				if (k == "true") {
					var j = Util.creatCode(4);
					$(".input-pw").show().val(j).select();
					Util.ajax({
						url: "/view/addpassword",
						data: {
							viewPassword: j,
